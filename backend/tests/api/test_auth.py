...
async def test_google_callback_new_user(client: AsyncClient, session: AsyncSession):
    # Cleanup
    await session.execute(text("DELETE FROM invoice WHERE user_id IN (SELECT id FROM \"user\" WHERE email = 'test@example.com')"))
    await session.execute(text("DELETE FROM \"user\" WHERE email = 'test@example.com'"))
...
@pytest.mark.asyncio
async def test_google_callback_existing_user(client: AsyncClient, session: AsyncSession):
    # Cleanup
    await session.execute(text("DELETE FROM invoice WHERE user_id IN (SELECT id FROM \"user\" WHERE email = 'test@example.com')"))
    await session.execute(text("DELETE FROM \"user\" WHERE email = 'test@example.com'"))
    await session.commit()
